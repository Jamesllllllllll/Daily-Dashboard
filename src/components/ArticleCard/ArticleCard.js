                ...
                <CardActionArea component={Link} to={`/article/${id}`} disableRipple>
                    {!isLoading
                        ...
                        <CardMedia
                            ...
                            image={image}
                            alt={title}
                            sx={{ height: "15rem", objectFit: "cover" }}
                        />
                        : <Skeleton variant="rectangular" height={'15rem'} sx={{}} />
                    }
                    <CardContent>
                        <Typography variant="body1" component="div" sx={{textAlign: 'start'}}>
                            {!isLoading ? title : <Skeleton variant="text" width={'100%'} height={20} sx={{}} />}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                            {!isLoading ? description : <Skeleton variant="text" width={'100%'} height={20} sx={{}} />}
                        </Typography>
                    </CardContent>
                    <CardContent sx={{ borderTop: '0.075rem #BFBFBF solid', textAlign: 'start', display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '.25rem', paddingBottom: '.25rem' }}>
                        <Typography variant="body2" color="text.secondary" >
                            {!isLoading ? <>{formatPublishedAt(publishedAt)} • {author}</> : <Skeleton variant="text" width={'5vw'} height={20} sx={{}} />}
                        </Typography>
